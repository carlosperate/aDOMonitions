...
for all callout types and themes:
...
aDOMonitions is a JavaScript library that scans the DOM for admonition
markers in HTML generated by a static site generator without admonition
support.
It then transforms the corresponding HTML into styled,
...
- **Two trigger styles**, GitHub (`[!TYPE]` blockquotes) and Docusaurus
  (`:::type` fences)
- **Four bundled themes**, `default-*` (light, dark or auto),
  `github-*` (light, dark or auto), `material`, `docusaurus`
